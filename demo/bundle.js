...
function getActivePeriods(startYear, startSemester, durationSemesters) {
...
function getActiveYears(periods) {
  return [...new Set(periods.map((period) => period.year))].sort((a, b) => a - b);
}

function valueForYear(values, year) {
...
function calculateBudget(budget, p = parameters) {
  const periods = getActivePeriods(budget.startYear, budget.startSemester, budget.durationSemesters);
...
function consolidateBudgets(budgets, p = parameters) {
